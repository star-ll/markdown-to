...
const google_translate_api_1 = __importDefault(
...
const markdownIt = new markdown_it_1.default({
	...
	highlight: function (str, lang) {
		if (lang && highlight_js_1.default.getLanguage(lang)) {
			try {
				return highlight_js_1.default.highlight(str, { language: lang })
					.value;
			} catch (__) {}
		}
...
class MarkdownTo {
	constructor(rootDir, outDir, config) {
		...
		this.template = (0, fs_1.readFileSync)(
			path_1.default.join(__dirname, `./preset/preset.${config.type}`),
			{
				encoding: "utf-8",
			}
		);
	...
	check() {
		...
		(0, promises_1.rm)(this.outBaseDir, { recursive: true, force: true })
			.then(
				() => (0, promises_1.access)(this.outBaseDir),
				(err) => {}
			)
	...
	start() {
		(0, promises_1.readdir)(this.rootDir)
			.then((fileNames) => {
				if (!Array.isArray(fileNames) || fileNames?.length <= 0) {
					throw new Error("空目录");
				}
